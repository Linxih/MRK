...
Skeleton *BVHLoader::Load(const std::string &_filename, double _scale)
{
  ...
  while (!file.eof())
  {
    ...
    for (unsigned int i = 0; i < nodes.size(); ++i)
    {
      ...
      auto fix = ignition::math::Matrix4d(ignition::math::Quaterniond(xAxis, IGN_DTOR(90)));
      auto fiy = ignition::math::Matrix4d(ignition::math::Quaterniond(yAxis, IGN_DTOR(90)));
      pos.SetTranslation(translation);
      transform = pos * transform;
      if(node->GetName() == "PELVIS" || node->GetName() == "L_HIP" || node->GetName() == "R_HIP")
      {
        transform = fix.Inverse() * transform * fix * fix;
        transform = fiy.Inverse() * transform * fiy;
      }
      else if(node->GetName() == "Menschmodell_50_Perzentil_blau_m_nnlich_deutsch" || node->GetName() == "ema_50percMaleMesh" || node->GetName() == "ROOT")
      {
        //auto fixhip = ignition::math::Matrix4d(ignition::math::Quaterniond(yAxis, IGN_DTOR(-90)));
        transform = transform;
      }
      /*else if(node->GetName() == "RSFoTa" || node->GetName() == "LSFoTa" || node->GetName() == "L_TOES" || node->GetName() == "R_TOES")
      {
        transform = fix.Inverse() * transform * fix;
        transform = fiyfoot.Inverse() * transform * fiyfoot;
